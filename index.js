...
const PORT = process.env.PORT || 3000;        // http服务订阅端口
const UUID = process.env.UUID || '9afd1229-b893-40c1-84dd-51e7ce204913';
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'choreo.jingchaowan.cn';          // 固定隧道域名,留空即启用临时隧道
let ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiNjdhNzU2OWQwY2Q4OWFhZmI3NDk5ZjNjZjNiYzlmNzMiLCJ0IjoiNGIyZWY5ODUtMTkwZS00ZGY5LTliNDUtMThhYWEzMDhmZjdhIiwicyI6Ik1tRm1OakF6TmpBdE5USTJNeTAwTVRGbUxUazJZelF0WTJOa016STRNalkxWldZMyJ9';              // 固定隧道密钥json或token,留空即启用临时隧道
const ARGO_PORT = process.env.ARGO_PORT || 8001;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
const CFIP = process.env.CFIP || 'saas.sin.fan';            // 节点优选域名或优选ip  
const CFPORT = process.env.CFPORT || 443;                   // 节点优选域名或优选ip对应的端口
const NAME = process.env.NAME || '';                        // 节点名称

// 自动解析和兼容 Base64 格式的 Cloudflare 凭证
try {
  if (ARGO_AUTH) {
    let rawAuth = ARGO_AUTH.trim();
    if (/^[A-Za-z0-9+/=]+$/.test(rawAuth)) {
      const decoded = Buffer.from(rawAuth, 'base64').toString('utf-8');
      if (decoded.trim().startsWith('{')) {
        rawAuth = decoded.trim();
      }
    }
    if (rawAuth.startsWith('{')) {
      const parsed = JSON.parse(rawAuth);
      if (parsed.a && parsed.t && parsed.s) {
        ARGO_AUTH = JSON.stringify({
          AccountTag: parsed.a,
...
app.get("/weblog", (req, res) => {
  try {
    ...
    if (fs.existsSync(logPath)) {
      ...
      res.send("web.log not found");
    }
  } catch (err) {
    res.send(`Error reading web.log: ${err.message}`);
  }
});

app.get("/botlog", (req, res) => {
  try {
    const logPath = path.join(FILE_PATH, 'bot.log');
    if (fs.existsSync(logPath)) {
      res.setHeader('Content-Type', 'text/plain; charset=utf-8');
      res.send(fs.readFileSync(logPath, 'utf-8'));
    } else {
      res.send("bot.log not found");
    }
  } catch (err) {
    res.send(`Error reading bot.log: ${err.message}`);
  }
});
